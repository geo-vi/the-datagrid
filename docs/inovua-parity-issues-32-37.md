...
| #35   | `defaultVisible=false` seeds internal state while `visible` is controlled; ordering works with either grid or parent ownership. Root sizing fallbacks, keep-flex/shared-space resize, batch callbacks, configurable handle/proxy geometry, menu visibility, and imperative size/fit APIs are runtime contracts. | The same ownership boundaries and sizing precedence are implemented across the header menu, optional toolbar, imperative API, filtering, editing, row/column virtualization, and locked-column geometry. A dedicated example and browser suite cover controlled/uncontrolled state plus a 10k × 36 production performance fixture. | No remaining delta against issue #35's acceptance criteria.                                                                                                                   |
