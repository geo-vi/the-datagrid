...
export function toggleSortInfo(opts: {
  ...
  if (currentDir === 0) nextDir = defaultDir
    ...
    nextDir = (defaultDir === 1 ? -1 : 1) as 1 | -1
  else if (currentDir === (defaultDir === 1 ? -1 : 1))
    nextDir = allowUnsort ? 0 : defaultDir
  else nextDir = defaultDir

  if (!multi) {
    return nextDir === 0
      ? null
      : ({ name, dir: nextDir } satisfies TypeSingleSortInfo)
  }
...
export function applyLocalSort(data: any[], sortInfo: TypeSortInfo): any[] {
  ...
  sorted.sort((a, b) => {
    for (const s of list) {
      const av = (a as any)?.[s.name]
      const bv = (b as any)?.[s.name]
